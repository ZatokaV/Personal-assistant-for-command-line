...
class Notification:
    ...
    def find_text(self, key_phrase):
        ...
        match = re.fullmatch(key_phrase, value)
        if match:
            return f"Key-phrase:{key_phrase} was found in the {value}"
        return f"{key_phrase} not found"
    ...
    def find_tag(self, tag):
        ...
        if tags:
            for each_tag in tags:
                # Перевірка тегу серед всіх тегів
                match = re.fullmatch(tag, each_tag.value)
                if match:
                    return "Tag found"
                return f"Sorry but there is no tag:{tag}"
        return "There are no tags at all"
    ...
    def delete_tags(self, input_tag):

        tags = self.tags
        if tags:
            try:
                self.tags.remove(Tag(input_tag))
                return f"{input_tag} was deleted successfully"
            except ValueError:
                return f"Sorry but there is no tag:{input_tag}"
        return f"There are no tags at all"
